...
1. **無投放項目**：非封存 LI 數量 = 0（含無 LI、底下 LI 全部封存）
2. **N / M 投放中**：有任一非封存 LI 尚未 `completed`（N = `active` LI 數，M = 全部 LI 總數，不含封存中的 `draft`）
3. **已完成**：底下所有非封存 LI 皆為 `completed` 或 `stop`，或為（`paused` 且 `endDate < 今天`）
...
| `draft`     | 草稿，尚未啟用                     | ❌       |
| `active`    | 啟用中                             | ✅       |
| `paused`    | 使用者主動暫停，可重新啟用或停刊   | ❌       |
| `stop`      | 使用者手動停刊的終態（中文：停刊） | ❌       |
| `completed` | 走期到期，系統自動收斂的終態       | ❌       |

> **封存（archived）為獨立 flag**，非 `status` 值，以 `archived_at` 欄位記錄。`draft`、`stop`、`completed` 狀態的 LI 可封存；`active`、`paused` 不可。封存後列表預設隱藏、**可還原**。`draft` 另支援軟刪除（確認 dialog 後使用者不可見且不可還原，DB 保留 `deleted_at`）。
...
> 封存（archived flag）為獨立維度，不出現在狀態轉換圖中。`draft`、`stop`、`completed` 皆可封存（可還原）；`draft` 另可軟刪除（見 §3.2）。

- `draft`：可**軟刪除**（確認 dialog，使用者不可見且不可還原，DB 保留記錄）、可封存，或啟用
- `active`：不可封存或刪除
- `paused`：不可直接封存，需先停刊（→ `stop`）或重新啟用（→ `active`）
- `stop` 為使用者觸發終態：不可啟用或修改，可封存以整理檢視（見 §3.3.1）
- `completed` 為系統觸發終態：不可啟用或修改，可封存以整理檢視（見 §3.3.1）

#### §3.3.1 封存與複製

**封存（archived flag）**

- 手動觸發，條件：LI 狀態為 `draft`、`stop`、`completed`
- 封存後列表預設隱藏，可透過篩選 / tab 查看
- **可還原**：還原後回到封存前的狀態（`draft` / `stop` / `completed`）
- `active`、`paused` 狀態的 LI 不可直接封存
...
- `stop` LI 走期到期後**維持 `stop`**，不自動 `completed`
- **預算花完不等於 completed**：仍在走期內加預算即可續跑
- `stop` / `completed` **為功能終態**：不可啟用、不可修改，需重新投放請建立新 LI 或複製（見 §7.2.3、§8.3.2）
...
- **封存中（`archived_at` 不為空）的 LI 不計入預算加總**；非封存的 `draft`、`paused`、`stop`、`completed` LI 皆計入
- **封存中的 LI 花費仍計入 Campaign 花費加總**：花費為已實際發生的成本，封存後仍反映於 Campaign 進度
...
| 選項     | 走期                                   | 每日預算表                                                |
| -------- | -------------------------------------- | --------------------------------------------------------- |
| **確認** | 套用走期變更                           | 延長→新增日預設 `null`（待分配），縮短→砍掉日退回待分配池 |
...
篩選／搜尋：狀態篩選（all / active / paused / draft / stop / completed / 已封存）+ 搜尋框（LI 名稱、LI ID、所屬廣告活動名稱、廣告活動 ID、所屬活動群組名稱、活動群組 ID）。
...
篩選／搜尋：狀態篩選（all / active / paused / draft / stop / completed / 已封存）+ 搜尋框（LI 名稱、LI ID）。
...
| 狀態        | 可執行操作    | 行為                                                                                   |
| ----------- | ------------- | -------------------------------------------------------------------------------------- |
| `draft`     | 軟刪除 / 封存 | 軟刪除：確認 dialog 後使用者不可見且不可還原（DB 保留 `deleted_at`）；封存：隱藏可還原 |
| `active`    | —             | 不可刪除或封存                                                                         |
| `paused`    | —             | 不可直接封存，需先停刊（→ `stop`）                                                     |
| `stop`      | 封存 / 複製   | 封存：列表隱藏，可透過篩選 / tab 查看，可還原；複製：建立新 `draft` LI（見 §8.3.2）    |
| `completed` | 封存 / 複製   | 同上                                                                                   |
...
| 欄位         | 說明                                                                 |
| ------------ | -------------------------------------------------------------------- |
| 名稱         | 素材包名稱；旁附連結按鈕，點擊後開新分頁至 Studio 外部預覽頁         |
| 廣告格式     | 素材包的廣告格式                                                     |
